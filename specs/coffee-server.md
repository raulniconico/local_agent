...
The field list is `prompts.BEAN_FIELD_NAMES`: **name, origin, region, variety, altitude, roaster, producer, farm, process, roast_date, note** — the response `fields` object, the Anthropic output schema and the Qwen key list are all generated from it, so adding a field means editing that tuple and `schemas.BeanFields`, and nothing else here.

**It is a hand-written copy of `coffee_can.repo.LABEL_FIELDS`**, since this server does not import coffee-can. Same two exclusions from `BEAN_FIELDS`, for the same reasons: the eleven flavour axes are scored from brews rather than printed on a bag, and `frozen_date` is the day the bag's *owner* put it in a freezer, which no roaster can know. `farm` joined both lists on 2026-08-26, with prompt wording that keeps it apart from `producer` — the grower is a person or a cooperative, the farm is the estate, finca, washing station or mill — because a label that prints only one had been getting it copied into both. `region` joined on 2026-08-29.

**Every name on that tuple needs a sentence in `LABEL_OCR`, and `region` went two revisions without one.** The tuple makes the model *return* a key; only the prompt says what the key means, and a key defined by its name alone is answered from its name. So a bag reading "Ethiopia Yirgacheffe" came back with the whole string in `origin` and `region` empty, or with "Ethiopia" in both — not a vision failure, and not something a better model fixes: the split into a country plus a subdivision belongs to the *client's* two-level form (`Choices.ORIGINS`, then `Regions.forOrigin`), which the model was never told about. Corrected 2026-09-03: `origin` and `region` are defined together as one instruction — `origin` is the country and nothing else, `region` is the area inside it, a combined line is to be split, a country is never repeated into `region`, and a farm or washing-station name belongs in `farm`. The same wording went into `claude_ocr.py` and `qwen_ocr.py`, whose prompts had the identical hole.
