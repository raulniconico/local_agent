...
| `farm` | TEXT | the estate or washing station a lot came from — beside `producer`, not instead of it. Storage-only (2026-08-26) |
| `frozen_date` | TEXT | ISO-8601 day the bag went into the freezer, NULL for one that did not. Storage-only (2026-08-26) |
| `roast_level`, `color_value`, `weight_loss`, `expansion_rate` | TEXT | the roast block. Storage-only (2026-08-24) |
...
**Returned shapes.** `guess_bean_fields` returns `{field: str}` over **`repo.LABEL_FIELDS`** — what a bag label can state — empty string where absent. That list is `BEAN_FIELDS` minus the eleven flavour axes and `flavor_source` (scored from brews, never printed) and minus `frozen_date` (the day the bag's *owner* froze it, which no roaster can know). It exists as one derived tuple rather than three copies of a `"flavor_" not in field` filter because that substring test silently let `frozen_date` through the day the column was added, and all three modules began asking a vision model to read a freezer date off a coffee bag. `coffee_server/prompts.BEAN_FIELD_NAMES` is a hand-written copy of the same list — a pair that has to move together. `suggest_brew` returns `{"summary": str, "dose_g": float|None, "grind_size": str, "stages": [{"temperature_c", "water_g", "time_seconds", "circling"}]}`; `transcribe_brew_session` returns the same plus `"dripper": str`. Both normalise defensively — neither endpoint's JSON mode guarantees a shape, and the Qwen modules strip markdown fences before parsing.
