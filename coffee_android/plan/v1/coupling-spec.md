...
| The `action`/`secondaryAction` slot, or what `SectionHeader` puts in it | `SectionActionSlack` assumes a **48dp** `TextButton` against a **24dp** text box, and subtracts the difference from the gap. Swap the button for anything of another height and every heading with a verb silently drifts from every heading without one — the failure this constant exists to stop, and one no golden catches on a heading whose action happens to be absent in the fixture | `V2` |
